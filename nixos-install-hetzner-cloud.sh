...
#! nix-shell -i bash -p git nixFlakes

# Script to install NixOS from the Hetzner Cloud NixOS bootable ISO image.
# (tested with Hetzner's `NixOS 21.05 (amd64/minimal)` ISO image).
#
...
nixos-install --root /mnt --flake /mnt/etc/nixos#nixmail --no-root-passwd
